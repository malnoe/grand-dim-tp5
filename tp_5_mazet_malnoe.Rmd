...
for(i in 1:nrep){
  # On simule de nouvelles valeurs pour cnt suivant le modèle, avec 
  #lambda_i = l'intensité pour l'obs i = lambda_hat[i]
  y_sim <- rpois(n, lambda = lambda_hat)
  
  # Ajustement d'un modèle LASSO avec lambda = lambda.1se sur l'ensemble des variables predictive
  ...
  for(name in names(borne.inf_i)){
    if(name %in% names(beta_true)){
      ...
      if (beta_true_j >= inf && beta_true_j <= sup) {
        ...
        # on augmente le compteur de couverture de 1.
